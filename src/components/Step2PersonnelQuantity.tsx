...
export const Step2PersonnelQuantity: React.FC<Step2PersonnelQuantityProps> = ({
  ...
  const handleSaveRename = (oldName: string) => {
    ...
    if (!trimmed || trimmed === oldName || columns.includes(trimmed)) {
      setEditingCol(null);
      setEditColValue('');
      return;
    }
    ...
    setMatrix((prev) => {
      ...
      Object.entries(prev).forEach(([paradero, cols]) => {
        ...
        next[paradero] = updatedCols;
      });
      syncToDraft(next, nextCols);
      return next;
    });
  ...
  const handleRemoveColumn = (colToRemove: string) => {
    ...
    setMatrix((prev) => {
      const next = { ...prev };
      Object.keys(next).forEach((paradero) => {
        if (next[paradero]?.[colToRemove]) {
          const updated = { ...next[paradero] };
          delete updated[colToRemove];
          next[paradero] = updated;
        }
      });
      syncToDraft(next, nextCols);
      return next;
            ...
            <thead>
              <tr className="bg-[#EBF3F9] text-[#173B56] border-b border-sky-200/70">
                ...
                {columns.map((col) => (
                  <th
                    key={col}
                    className="py-2.5 px-1 text-center font-bold text-xs sm:text-sm tracking-tight min-w-[46px] group"
                  >
                    {editingCol === col ? (
                      <div className="flex items-center justify-center gap-0.5">
                        <input
                          ...
                          className="w-12 h-6 px-1 text-center font-black text-xs text-[#173B56] bg-white border-2 border-[#00843D] rounded-md shadow-2xs outline-none"
                        />
                      </div>
                    ) : (
                      <div className="flex items-center justify-center gap-0.5">
                        <button
                          ...
                          className="text-[#173B56] font-extrabold text-xs sm:text-sm hover:text-[#00843D] hover:underline cursor-pointer flex items-center gap-0.5 group-hover:scale-105 transition-transform"
                        >
                          <span>{col}</span>
                          <Edit2 className="w-2.5 h-2.5 opacity-0 group-hover:opacity-100 text-gray-400 transition-opacity" />
                        </button>
                        ...
                        {columns.length > 1 && (
                          <button
                            ...
                            className="text-gray-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 ml-0.5"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        )}
